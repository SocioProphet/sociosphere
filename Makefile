...
.PHONY: validate validate-standards estate-control-graph-validate

validate: validate-standards estate-control-graph-validate
	@echo "OK: validate"
...
estate-control-graph-validate:
	python3 -m pip install --user pyyaml >/dev/null
	python3 tools/check_estate_control_graph.py
...
registry-validate:
	...
	@echo "==> Validating estate control graph..."
	python3 -m pip install --user pyyaml >/dev/null
	python3 tools/check_estate_control_graph.py
...
workspace-check: validate registry-validate compliance-check lock-verify topology-check hygiene-check estate-control-graph-validate
