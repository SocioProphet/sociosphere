...
reasoning artifacts emitted by Superconscious without executing agents, invoking
models, or requiring raw private reasoning content.
"""
...
def validate_reasoning_dir(run_dir: Path) -> Dict[str, Any]:
    ...
    for index, event in enumerate(events, start=1):
        ...
        if any(token in summary.lower() for token in ["api_key", "password", "private key", "token="]):
            errors.append(f"event line {index} summary appears to contain sensitive material")
