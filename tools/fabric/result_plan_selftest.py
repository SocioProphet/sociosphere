...
class ResultPlanSmokeTest(unittest.TestCase):
    def test_direct_serving_decisions(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse(stale_plan.include_content)
            self.assertEqual(tombstone_plan.serve_mode, "omit")
            self.assertFalse(tombstone_plan.include_metadata)
            self.assertEqual(authority_plan.serve_mode, "defer_until_refresh")

    def test_matrix_serving_decisions(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            plans = serving_decisions_from_planner_matrix(planners)
            self.assertEqual(plans["tombstone"]["serve_mode"], "omit")
            self.assertEqual(plans["stale_mirror"]["serve_mode"], "metadata_only")
            self.assertEqual(plans["authority_transition"]["serve_mode"], "defer_until_refresh")

    def test_cli_show_result_plan(self) -> None:
        cases = [
            argparse.Namespace(
                kind="stale_mirror",
                root=None,
