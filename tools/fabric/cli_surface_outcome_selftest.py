...
from .cli import cmd_show_surface


class CliSurfaceOutcomeSmokeTest(unittest.TestCase):
    def test_show_surface_for_reconcile_outcomes(self) -> None:
        cases = [
            (
                "stale_mirror",
                argparse.Namespace(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for idx, (_, ns) in enumerate(cases):
                ...
                ns.root = str(root)
                rc = cmd_show_surface(ns)
                self.assertEqual(rc, 0)
                self.assertTrue((root / "events.ndjson").exists() or (root / "tombstone" / "events.ndjson").exists())
